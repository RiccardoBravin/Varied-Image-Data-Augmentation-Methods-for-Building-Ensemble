%method with DCT that return a new training set and a new training label
%starting from old training set, old training label and the dimension of
%training set

function[trainingImages, y] = dataAugDct(images, x, DIM1)
for t=1:DIM1
        %apply three different methods
        images(:,:,:,t+DIM1) = method1dct(images(:,:,:,t));
        images(:,:,:,t+(2*DIM1)) = method2dct(images(:,:,:,t));
        ...
        images(:,:,:,t+(3*DIM1)) = method3dct(images(:,:,:,t),samples);
